...
#+BEGIN_COMMENT
At the algorithm that will be described is intended primarily
for [[https://en.wikipedia.org/wiki/Simple_polygon][simple polygons]] (it could potentially be used for weakly simple polygons
and multipolygons).
#+END_COMMENT
...
points/lines will be confined to B. We extract points PN,PE,PW,PS for
all extreme cardinal points.

Goal: We want to cut a corner C (also called subdivision) from P that
contains the nearest-to-road point as one of its edges. The cut will
be done using two lines, one vertical (we'll also call this
green-line) and one horizontal (we'll also call this the red-line). We
want C to be of a given area A.


Now get the closest point on R to P (we call this Q, this is the
nearest point on the road to the polygon).

#+BEGIN_COMMENT
From here on, we convert B to a point grid G (also called a fishnet),
and we'll search for points in G such that we can do the split we
want.
#+END_COMMENT

We take a sweeping-line approach (this will be the green-line). This
sweeping line will move in the opposite direction to the
nearest-to-road-point.

Because we anticipate C to have a rectangular shape, we give the
sweeping-line a head-start of sqrt(A).

The sweeping line will advance until the covered area is greater or
equal to A, after which it will stop (if it goes beyond P's cover,
we can conclude there is no C).

When the green-line is in place, we now have to position the red-line
and we can do that with the left-side fixed, and just look for the
right spot for the right side.
...
- [X] Need to sort clock-wise to determine cardinal directions
      ...
      which corner to cut.
- [ ] Get green-line sweep trajectory
- [ ] Split on green-line
- [ ] Update the cut polygon and insert the corner in the DB
- [ ] Need to look again at specs and write down with examples, when the red-line cut
      will be used.
* Other
