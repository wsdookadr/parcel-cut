...
In what follows, we assume the north-west corner needs to be cut (if
it's a different corner, the cut search will be run, the cut line
found and then all the shapes rotated back).

We place an inset (a horizontal line) that will be located sqrt(A) to
the south (relative to the north edge). The inset is located at sqrt(A)
...
search for a better position for it, using binary search.
* Implementation
** Visualization
